...
// 2. Creamos un objeto para mapear iconos por nombre
const Icons = {
  FileText,
  PenTool,
  FileCheck,
...
export default function IconShowcaseLicitaciones() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isHovered, setIsHovered] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  // 3. Extraemos t() para leer circleServices
  const { t } = useTranslation();

  const circleTitle = t("consultoriaLicitaciones", "circleTitle");
  const licitacionesServices = t("consultoriaLicitaciones", "circleServices");

  // Convertimos en array si no lo es
  const servicesArray = Array.isArray(licitacionesServices)
    ? licitacionesServices
    : [];
